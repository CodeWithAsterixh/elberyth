...
<template>
    <div class="w-full flex flex-col gap-2">
        <span class="w-full aspect-square min-h-52 overflow-hidden relative  group isolate bg-secondary flex items-center justify-center">
            ...
            <div class="w-fit p-2 px-0 absolute flex flex-col gap-2 top-0 left-0 z-10">
            ...
            <div v-if="cart.items.find(p => p.productDetails.id === product.id)" class="shadow-md absolute left-1/2 -translate-x-1/2 flex items-center  gap-1 min-[498px]:gap-3 justify-center  text-light duration-300 delay-[250ms] -bottom-[100%] group-hover:bottom-2">
        ...
        <ul class="w-full flex flex-col gap-2">
            <li>
                <NuxtLink :href="`/shop/${product.id}`" class="hover:text-primary hover:underline hover:underline-offset-2 duration-300">{{ product.name }}</NuxtLink>
            </li>
